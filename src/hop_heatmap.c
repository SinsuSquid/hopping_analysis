...
void hop_heatmap(){
	printf("\tNow analyzing heatmap\n");

	double binsize = (double)MAX_H / NUMBINS;
	fprintf(fp_out, "%f", 0.0);
	for (int i = 0; i < NUMBINS; i++) fprintf(fp_out, "\t%f", (i + 0.5) * binsize);
	fprintf(fp_out, "\n");

	int **heatmap = (int **)malloc(sizeof(int *) * NUMBINS);
	for (int i = 0; i < NUMBINS; i++)
		heatmap[i] = (int *)malloc(sizeof(int) * NUMBINS);
	...
	for (int t = deltaT; t < numTraj - deltaT; t++){
		...
		for (int i = 0; i < numLi; i++){
			...
			for (int j = 0; j < numLi; j++){
				...
				if (hop[t][j] > MAX_H) continue;
				int jdx = (int)(hop[t][j] / binsize);
	...
	for (int i = 0; i < NUMBINS; i++){
		...
		for (int ii = 0; ii < NUMBINS; ii++){
			fprintf(fp_out, "\t%lf", (double)heatmap[i][ii] / numPoints);
		}
		fprintf(fp_out, "\n");
	}
