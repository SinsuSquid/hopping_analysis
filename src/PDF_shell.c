...
void PDF(){
	...
	fprintf(fp_out, "#\tr\ttotal");
	for (int i = 0; i < NUMSHELL; i++){ fprintf(fp_out, "\t%d-th, eq\t%d-th, hop", i, i); }
	fprintf(fp_out, "\n");
	...
	for (int t = deltaT; t < numTraj - deltaT; t++){
		...
		for (int i = 0; i < numLi; i++){
			...
			for (int j = 0; j < numLi; j++){
				...
				int shellIdx = shell_assign(distance);
				int idx = (int)(distance / binsize);
				// printf("i = %d\tj = %d\tshellIdx = %d\tidx = %d\thopped = %d\n", i, j, shellIdx, idx, hopped);
				if (idx < NUMBINS){
					PDF[0][idx] += 1; numPoints[0] += 1;
					if (shellIdx) { PDF[shellIdx * 2][idx] += 1; numPoints[shellIdx * 2] += 1;} 
					if (hopped){
						PDF[1][idx] += 1; numPoints[1] += 1;
						if (shellIdx) { PDF[2 * shellIdx + 1][idx] += 1; numPoints[2 * shellIdx + 1] += 1; }
					}
				}
			}
		}
