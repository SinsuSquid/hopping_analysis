...
void survival(){
	printf("\tNow calculating survival probability...\n");
	fprintf(fp_out, "#\tt\tC_s(t)\n");

	for (int dt = 0; dt < (int)(numTraj / 3); dt += DELTA_T){
		if (!(dt % 100)) printf("\t\tNow t = %d\n", dt);
		long hit = 0; long total = 0;
		for (int start = deltaT; start < numTraj - deltaT - dt; start++){
			for (int i = 0; i < numLi; i++){
				total++;
				int survived = 1;
				for (int ii = 1; ii < dt; ii++){
					if (hop[start + ii][i] > 2.5){ survived = 0; break; } }
				hit += survived;
			}
		}
		fprintf(fp_out, "%d\t%lf\n", dt, (double)hit / total);
