...
void PDF(){
	printf("\tCalculating distance PDF ...\n");

	fprintf(fp_out, "#\tr\ttotal, eq\ttotal, hop");
	for (int i = 1; i < NUMSHELL+1; i++){ fprintf(fp_out, "\t%d-th, eq\t%d-th, hop", i, i); }
	fprintf(fp_out, "\n");

	// i = 0 is for PDF_eq
	double **PDF = (double **)malloc(sizeof(double *) * NUMSHELL * 2);
	int *numPoints = (int *)malloc(sizeof(int) * NUMSHELL * 2);
	for (int i = 0; i < NUMSHELL * 2; i++){
		PDF[i] = (double *)malloc(sizeof(double) * NUMBINS);
		numPoints[i] = 0;
		for (int ii = 0; ii < NUMBINS; ii++) PDF[i][ii] = 0.0;
	}
	...
	for (int t = deltaT; t < numTraj - deltaT; t++){
		...
		for (int i = 0; i < numLi; i++){
			// Searching n-th nearest neighbor of i-th Li
			double nearest[NUMSHELL];
			for (int ii = 0; ii < NUMSHELL; ii++) nearest[ii] = 999999999.0;

			// i-th atom hopped?
			int hopped = 0;
			if (hop[t][i] > H_STAR) { hopped = 1; }

			for (int j = 0; j < numLi; j++){
				if (i == j) continue;
				double distance = sqrt(getDistance(coord[t][i], coord[t][j]));
				int idx = (int)(distance / binsize);
				if (idx < NUMBINS){ 
					PDF[0][idx] += 1; numPoints[0] += 1;
					if (hopped){ PDF[1][idx] += 1; numPoints[1] += 1; }
				}
				for (int ii = 0; ii < NUMSHELL; ii++){
					if (nearest[ii] > distance){
						for (int iii = NUMSHELL - 1; iii > ii; iii--) nearest[iii] = nearest[iii-1]; // Push it
						nearest[ii] = distance; // Assign
						break;
					}
				}
			}

			// for (int ii = 0; ii < NUMSHELL; ii++) printf("nearest[%d] = %lf\t", ii, nearest[ii]);
			// printf("\n");

			for (int ii = 1; ii < NUMSHELL; ii++){
				int idx = (int)(nearest[ii - 1] / binsize);
				if (idx < NUMBINS){
					PDF[ii * 2][idx] += 1; numPoints[ii * 2] += 1;
					if (hopped){ PDF[ii * 2 + 1][idx] += 1; numPoints[ii * 2 + 1] += 1;}
				}
			}
		}
	}
