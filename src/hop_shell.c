...
void initialize();
void hop_function();
int shell_assign(double);
void hop_shell();

int main(int argc, char *argv[]){
	if (argc != 4){
		printf("USAGE : ./hop_shell.x ***.lammpstrj DELTA_T hop_shell.out\n");
		exit(1);
	}
...
void hop_shell(){
	printf("\tNow analyzing h for n-th neighbors...\n");
	fprintf(fp_out, "#\th\ttotal\t1st\t2nd\t3rd\t4nd\t5+\n");

	double binsize = (double)MAX_H / NUMBINS;

	double **hs = (double **)malloc(sizeof(double*) * NUMSHELL);
	int **numPoints = (int **)malloc(sizeof(int *) * NUMSHELL);

	for (int i = 0; i < NUMSHELL; i++){
		hs[i] = (double*)malloc(sizeof(double) * NUMBINS);
		numPoints[i] = (int *)malloc(sizeof(int) * NUMBINS);
	...
	for (int t = deltaT; t < numTraj - deltaT; t++){
		...
		for (int i = 0; i < numLi; i++){
			if (hop[t][i] > MAX_H) continue;
			int idx = (int)(hop[t][i] / binsize);
			for (int j = 0; j < numLi; j++){
				if (i == j) continue;
				double distance = sqrt(getDistance(coord[t][i], coord[t][j]));
				int shellNum = shell_assign(distance);
				// printf("i = %d\t", i);
				// printf("distance = %lf, shellNum = %d\t", distance, shellNum);
				// printf("hop[%d][%d] = %lf\n", t, j, hop[t][j]);

				hs[0][idx] += hop[t][j]; numPoints[0][idx]++;
				if (shellNum){
					hs[shellNum][idx] += hop[t][j];
					numPoints[shellNum][idx]++;
				}
			}
		}
	}
	...
	for (int i = 0; i < NUMBINS; i++){
		double h = (i + 0.5) * binsize;
		fprintf(fp_out, "%lf", h);
		for (int ii = 0; ii < NUMSHELL; ii++){
			fprintf(fp_out, "\t%lf", hs[ii][i] / numPoints[ii][i]);
		}
		fprintf(fp_out, "\n");
	}
...
int shell_assign(double distance){
	if (distance > 17.31794625) return 0;
	else if (distance > 14.46639675) return 5;
	else if (distance > 11.54529775) return 4;
	else if (distance > 8.55464825) return 3;
	else if (distance > 5.285799) return 2;
	else return 1;
}
